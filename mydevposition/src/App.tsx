...
function App() {
  ...
  const getResultContent = () => {
    ...
    if (resultPosition === "프론트엔드") {
      ...
      return "어디서 무엇을 하든 항상 묵묵하게 하자! 힘들어도 끈기 있게 맡은 일을 끝까지 해내는 당신이 진정한 외유내강. 데이터베이스, 서버 개발처럼 보이지 않는 곳에서 꾸준하게 노력하는 백엔드 포지션을 추천합니다. 멋쟁이 사자처럼에서 백엔드 개발자로 활약할 당신의 모습이 벌써 기대되는군요:)";
    } else if (resultPosition === "기획") {
      return "내가 바로 대문자 J. 계획 없이 사는 것은 절대 있을 수 없는 일이야! 플랜 A부터 Z까지 꼼꼼하고 확실하게 해 두는 것을 선호하는 당신에게 기획 포지션을 추천합니다. 프로젝트는 코딩을 잘 하는 사람만 있으면 된다고 생각하기 쉽지만, 훌륭한 기획자가 있어야 그것을 구현하는 개발자들이 존재할 수 있는 법. 멋쟁이 사자처럼에서 아이디어 뱅크로 활약할 당신을 기대할게요!";
    } else if (resultPosition === "디자인") {
      return "책을 살 때도, 식당을 고를 때도 언제나 외관을 먼저 보게 되는 당신! 예쁘고 효율적인 디자인을 싫어하는 사람은 없어. 아무리 잘 만든 프로젝트여도 디자인이 별로라면 활용도가 떨어지는게 당연한거 아니야? 이런 당신에게 미적 감각이 뛰어나야 하는 것은 물론, 창의력과 상상력까지 겸비해야 맡을 수 있는 디자인 포지션을 추천합니다. 멋쟁이 사자처럼에서 당신의 감각을 보여주세요!";
    }
